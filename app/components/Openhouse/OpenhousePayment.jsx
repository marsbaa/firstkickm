...
class OpenhousePaymentForm extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      payer: [],
  ...
  formSubmit(e) {
    ...
    this.state.payer.map((student, id) => {
      if (student.joining) {
        dispatch(updateOpenhouseRegistration(student.key));
        let response = dispatch(addOpenhouseStudent(student));
  ...
  render() {
    ...
    this.state.payer.map((student, id) => {
      if (student.joining) {
        email = student.email;
        tabs.push(
          <Tab eventKey={count} key={id} title={student.childName}>
        ...
        fees.push(
          ...
          >
            <Col xs={12} md={12} style={{ borderBottom: '1px solid #bcbcbc' }}>
              <p style={{ fontWeight: '800', marginBottom: '0px' }}>
                <Glyphicon glyph="user" /> {student.childName}
              </p>
            </Col>
          </Row>
        );
        ...
        if (student.deposit !== undefined) {
          if (student.deposit !== '0') {
            fees.push(
              <Row
                key={'deposit' + student.childName}
                style={{ padding: '0px 15px', marginBottom: '5px' }}
              >
                <Col xs={8} md={8}>
                  <b style={{ color: '#1796d3' }}>Deposit</b>
                </Col>
                <Col xs={4} md={4} style={{ float: 'right' }}>
                  <p style={{ textAlign: 'right', marginBottom: '0px' }}>
                    (${student.deposit})
...
export default connect(mapStateToProps)(OpenhousePaymentForm);
