...
import {getBreakDown, getTotalSessions, getCalendarKey, checkEarlyBird, getPerSession} from "helper";
...
class PaymentBreakdown2 extends React.Component {
  ...
  render() {
    ...
    Object
      ...
      .map((payerKey, index) => {
        ...
        if (sessionDates !== undefined) {
          ...
          const actualTerms = calendars[getCalendarKey(payers[payerKey], classes)].terms;
          let earlybird = checkEarlyBird(actualTerms, sessionDates, this.state.receivedDate);
          //Display Term Fee
          let paymentDetail = {};
          let childTotal = 0;
          html.push(<ChildPayDetails
            key={childName}
            childName={childName}
          ...
          if (count > 0 && totalSessions > 4) {
            html.push(<SiblingDiscount key={"siblingdiscount" + childName} count={count} amount={sd}/>);
            paymentDetail.siblingDiscount = true;
            paymentDetail.siblingDiscountAmount = count > 1
              ? sd[1]
              : sd[0];
            childTotal -= count > 1
              ? sd[1]
          ...
          if (studentCredits.length > 0) {
            let totalCredit = reduce(studentCredits, function (sum, n) {
              return sum + n.amount;
            }, 0);
            html.push(
              <Row
    ...
    return (
      <div>
        <Panel
          header={< font style = {{ fontSize: "16px", fontWeight: "bold" }} > Fees Breakdown < /font>}
          footer={< Row style = {{ paddingRight: "20px" }} > <Col xs={8} md={8} lg={8}>
          <font
            style={{
            fontSize: "16px",
            fontWeight: "bold"
