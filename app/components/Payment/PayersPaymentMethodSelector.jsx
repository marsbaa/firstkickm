...
class PayersPaymentMethodSelector extends React.Component {
  ...
  formSubmit() {
    const { dispatch, paymentDetails, childDetails, receivedDate } = this.props;
    let finalPaymentDetails = [];

    Object.keys(childDetails).map(payerKey => {
      if (paymentDetails[payerKey].credits !== undefined) {
                    ...
                    {this.state.paymentMethod === "NETS" ? (
                      ...
                      >
                        ...
                        this.state.chequeNumber !== "" ? (
                          <HelpBlock>Please enter reference number</HelpBlock>
                        ) : null}
                      </FormGroup>
                    ) : null}
