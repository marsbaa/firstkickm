...
class PaymentReport extends React.Component {
  ...
  render() {
    ...
    Object.keys(classes).forEach((classKey)=> {
      ...
      var filteredStudents= _.filter(students, {'venueId': selection.id, 'currentClassDay': _.capitalize(day), 'currentClassTime': classTime, 'ageGroup' : ageGroup})
      filteredStudents = _.filter(filteredStudents, (o) => {
       ...
       if (_.size(paid) !== 0 || _.size(unpaid) !== 0) {
         ...
         html.push(
           <Row key={ageGroup+classTime+day} style={{backgroundColor: '#656565', padding: '0px 15px', color: '#ffc600'}}>
              <Col xs={12} md={12}>
       ...
       //Progress Bar Value
       const now = Math.round(_.size(paid) / (_.size(paid)+_.size(unpaid)) * 100)

       //Display Paid List
       if (_.size(paid) !== 0) {
         html.push(
           <Row
             key= {"paidlist"+ageGroup+classTime+day} style={{backgroundColor: '#f5f5f5', padding: '10px 15px', borderBottom: '1px solid #cccccc', fontSize: '12px', fontWeight: '800'}}>
             <Col xs={6} md={6}>
               Paid <Badge>{_.size(paid)}</Badge>  Amount <Badge>${paidAmount}</Badge>
             </Col>
             <Col xs={6} md={6}>
       ...
       if (_.size(unpaid) !== 0) {
         ...
         html.push(
           <Row
             key= {"unpaidlist"+ageGroup+classTime+day} style={{backgroundColor: '#f5f5f5', padding: '10px 15px', borderBottom: '1px solid #cccccc', fontSize: '12px', fontWeight: '800'}}>
             <Col xs={12} md={12}>
          ...
          Object.keys(unpaid).forEach((unpaidId) => {
            html.push(
              <PayerReport
                key={unpaid[unpaidId].key} student={unpaid[unpaidId]}
    ...
    html.push(
      <Row
        key={"studentsPaid"}
        style={{padding: '8px 20px', borderBottom: '1px solid #cccccc', display: 'flex', alignItems: 'center'}}
      >
