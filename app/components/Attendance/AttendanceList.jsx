...
import {getActive, getNotActive, getCentreCalendarDates, sortByEndTime, filterByAMPM, classToday, isManager} from 'helper'
...
class AttendanceList extends React.Component{
  ...
  render() {
    var {students, searchText, selection, calendars, makeUps, auth, users} = this.props;
    var html=[];
    ...
    Object.keys(classes).forEach((classKey)=> {
      ...
      if (_.size(filteredActiveClass) !== 0) {
        ...
        var filteredMakeUps = _.filter(makeUps, {toCentre: selection.key, toDate: moment(this.state.startDate).format('YYYY-MM-DD'), ageGroup: ageGroup, toClassTimeDay: classTimeDay})
   ...
   return (
     <Grid style={{padding: '10px 0px'}}>
       ...
       {isManager(auth, users)? <Row style={{padding: '8px 10px', borderBottom: '1px solid #cccccc', display: 'flex', alignItems: 'center'}}>
         <Col xs={12} md={12}>
