  ...
  render() {
    ...
    coachSchedule.map(schedule => {
      if (schedule.date === date) {
        if (
          classTimingClash(schedule.classKey, classKey) &&
          schedule.classKey !== classKey
        ) {
      ...
      <div style={{ marginTop: '10px' }}>
        <Row>
          ...
          <Col xs={5} md={5} lg={5}>
            P
            {' '}
            <Badge style={{ backgroundColor: '#f5bb05', color: 'black' }}>
              {paid}
            </Badge>
            {numOfTrials === 0
              ? ''
              : <font> T <Badge>{numOfTrials}</Badge></font>}
          </Col>
        </Row>
        <Row>
