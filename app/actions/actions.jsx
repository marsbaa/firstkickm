      ...
      .then(snapshot => {
        ...
        Object
          ...
          .forEach(userId => {
            ...
            parsedUsers.push({key: userId, name: user.name, email: user.email, assignedCentres: user.assignedCentres, assignedRoles: user.assignedRoles});
          });
        ...
        trialsRef
          ...
          .then(snapshot => {
            ...
            trialList.forEach(trials => {
              var trialExist = _.findKey(firebaseTrialList, {childId: trials.child_id}) === undefined || ''
                ? false
                : true;
              if (!trialExist) {
                trialsRef
      ...
      .then(snapshot => {
        ...
        Object
          ...
          .map(id => {
            ...
            const formattedDate = '2017-' + date[1].slice(0, 3) + '-' + date[0].slice(0, 2);
            const startTime = splitS[2].slice(0, 1) + ':00' + splitS[2].slice(1, 3);
            parsedOpenHouse.push({
              key: id,
              ...openhouse[id],
              centreName: splitS[0],
              dateOfTrial: moment(formattedDate, 'YYYY-MMM-DD').format('YYYY-MM-DD'),
              startTime
            });
          });
  ...
  return dispatch => {
    ...
    attendanceRef
      ...
      .then(snapshot => {
        ...
        attendance.attended = attendance.attended === undefined || attendance.attended === false
          ? true
          : null;
        ...
        return attendanceRef.update({attended: attendance.attended, classId, sessionRate});
      })
      .then(() => {
...
export var addClass = (cla, centreKey) => {
  var classRef = firebase
    .database()
    .ref('/centres/' + centreKey + '/classes');
  var classKey = classRef
    .push()
    .key;
  var updates = {};
...
export var toggleSchedule = (classKey, date, val) => {
  var ScheduleRef = firebase
    .database()
    .ref('coachSchedule/' + classKey);
  var updates = {
    [date]: {
      assigned: val
    }
...
export var startCoachSchedule = () => {
  return dispatch => {
    var CoachScheduleRef = firebase
      .database()
      .ref('coachSchedule');
    var parsedSchedule = [];
    return CoachScheduleRef
      .once('value')
      .then(snapshot => {
        var value = snapshot.val();
        if (value !== null) {
    ...
    ageGroupRef
      ...
      .then(snapshot => {
        ...
        Object
          .keys(ageGroup)
          .forEach(ageGroupId => {
            parsedAgeGroup.push({key: ageGroupId, name: ageGroup[ageGroupId].name, minAge: ageGroup[ageGroupId].minAge, maxAge: ageGroup[ageGroupId].maxAge});
          });
